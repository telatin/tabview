...
export parser.parseDelimitedFile
export parser.parseDelimitedStream
...
proc initTUI(ctx: var nw.Context[State], data: TableData, schemeName: string,
             filename: string = "", hasHeader: bool = true) =
  # Select color scheme
  ...
  ctx.data.tableData = data
  ctx.data.filename = if filename.len > 0: filename else: "<data>"
...
proc viewTable*(data: TableData, schemeName: string = "default",
                filename: string = "", hasHeader: bool = true) =
  ## Launch the interactive TUI table viewer with in-memory TableData.
  ## `filename` is optional and only used for display in the status bar.
  var ctx: nw.Context[State]
  initTUI(ctx, data, schemeName, filename, hasHeader)
  runEventLoop(ctx)

proc viewTabularFile*(path: string, schemeName: string = "default",
                      delimiter: char = '\0', skipLines: int = 0,
                      skipPrefix: string = "", hasHeader: bool = true,
                      maxColWidth: int = 20) =
  ...
  if path == "-":
    ...
    # Disconnect from the pipe and reopen terminal for keyboard input
    when defined(posix):
    ...
    data = parseDelimitedStream(stringStream, delimiter, skipLines, skipPrefix, hasHeader, maxColWidth)
    stringStream.close()
    filename = "<stdin>"
  else:
    let detectedDelim = if delimiter == '\0': detectDelimiter(path) else: delimiter
    data = parseDelimitedFile(path, detectedDelim, skipLines, skipPrefix, hasHeader, maxColWidth)
    filename = path
